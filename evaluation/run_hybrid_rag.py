"""Arm N3 - STANDARD HYBRID RAG, built as a competent 2026 practitioner would.
dense(Annoy) + BM25(FTS5) fused by Reciprocal Rank Fusion, dedup by title,
k=5, neutral prompt that PERMITS parametric fallback. No cross-encoder, no
deep tier, no entity spans, no gate. Same model/corpus/hardware as arm B.
...
def main():
    ...
    retr = R.Retriever(); print("Loading index...", flush=True); retr.load()
    ...
    cfg_hash = hashlib.md5(json.dumps(
        {"arm":"N3","k":args.k,"pool":args.pool,"fusion":"RRF60",
         "dense":True,"bm25":True,"dedup":"title","reranker":False,
         "gate":False,"parametric_fallback":True}, sort_keys=True).encode()).hexdigest()[:12]

    rows = [json.loads(l) for l in open(os.path.join(C.SETS,args.set),encoding="utf-8") if l.strip()]
    if args.limit: rows = rows[:args.limit]
    path = os.path.join(C.RES,args.out+".jsonl")
    done = C.load_done(path); todo=[r for r in rows if r["qid"] not in done]
    print("%d in set | %d done | %d to run\n"%(len(rows),len(done),len(todo)),flush=True)
    C.write_manifest(args.out, {
        "arm":"N3","set":args.set,"set_md5":C.md5(os.path.join(C.SETS,args.set)),
        "config_hash":cfg_hash,"k":args.k,"pool":args.pool,
        "retrieval":"hybrid dense+BM25, RRF(k=60), dedup by title, no reranker, no gate",
        "system_prompt":SYS,"temperature":0.0,
        "note":"fair standard-practice RAG baseline"})
    ...
    for i, r in enumerate(todo, 1):
        ...
        try:
            ...
            if len(terms) > 3: terms = sorted(terms, key=retr._doc_freq)[:3]
            bm = []
            for joiner in (" AND "," OR "):
                ...
                try:
                    bm = [x["rowid"] for x in retr.db.execute(
                        ...
                        "ORDER BY bm25(passages_fts,10.0,1.0) LIMIT ?",(m,args.pool)).fetchall()]
                except Exception: bm=[]
                if bm: break
            fused = rrf([dense, bm])
            got, seen = [], set()
            if fused:
                ...
                by={x["id"]:x for x in retr.db.execute(
                    "SELECT id,title,text,source FROM passages WHERE id IN (%s)"%qm,fused).fetchall()}
                for pid in fused:
                    x=by.get(pid)
                    if x is None or x["title"] in seen: continue
                    seen.add(x["title"])
                    got.append({"title":x["title"],"text":x["text"],"source":x["source"]})
                    if len(got)>=args.k: break
            ctx="\n\n".join("[%s] %s"%(p["title"]," ".join(p["text"].split()[:120])) for p in got)
            out=llm.create_chat_completion(
                messages=[{"role":"system","content":SYS},
                          {"role":"user","content":"Passages:\n\n%s\n\nQuestion: %s"%(ctx,r["question"])}],
                max_tokens=128,temperature=0.0,stream=False)
            raw=out["choices"][0]["message"]["content"]
            txt=re.sub(r"<think>.*?</think>","",raw or "",flags=re.S).strip(); err=None
        except Exception as ex:
            txt,got,err="",[],"%s: %s"%(type(ex).__name__,ex)
        rec={"qid":r["qid"],"arm":"N3","config_hash":cfg_hash,
             "ts":time.strftime("%Y-%m-%dT%H:%M:%S"),"question":r["question"],
             "answers":r.get("answers"),"answer":txt,"error":err,
             "n_retrieved":len(got),"retrieved":got,"wall_s":round(time.time()-t0,2)}
        for k in ("prop","rel","s_pop","decile","src","subj"):
            if k in r: rec[k]=r[k]
        w.write(rec)
        print("  [%d/%d] %5.1fs eta %4.1fh  %s"%(i,len(todo),rec["wall_s"],
